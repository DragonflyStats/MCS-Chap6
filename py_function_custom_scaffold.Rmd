...
#### Description 
This function can be used to generate R wrapper for a specified Python function while allowing to inject custom code for critical parts of the wrapper generation, such as process the any part of the docs obtained from py_function_docs() and append additional roxygen fields. The result from execution of python_function is assigned to a variable called python_function_result that can
also be processed by postprocess_fn before writing the closing curly braces for the generated wrapper function.
...
#### Arguments
* ``python_function``: Fully qualfied name of Python function or class constructor (e.g. tf$layers$average_pooling1d)
* ``r_function``: Name of R function to generate (defaults to name of Python function if not specified)
* ``additional_roxygen_fields``: A list of additional roxygen fields to write to the roxygen docs, e.g. list(export = "", rdname = "generated-process_docs_fn
A function to process docs obtained from reticulate::py_function_docs(python_function).
* ``process_param_fn``: A function to process each parameter needed for python_funcion before executing
python_funcion.
